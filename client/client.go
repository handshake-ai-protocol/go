...
type Config struct {
	...
	Offline bool
	// AdminToken is the bearer token for receipt-read endpoints
	// (Registry locked down GET /v1/receipts/{id} in commit 12cb4f0).
	// Required when calling FetchReceipt / WaitForAnchor against a
	// Registry that enforces auth on receipt reads; harmless when
	// omitted against a Registry with anonymous reads enabled. Mirrors
	// packages/handshake-py/python/handshake/client.py:Handshake.admin_token
	// and packages/handshake-ts/ts/client.ts:HandshakeOptions.adminToken.
	AdminToken string
}
